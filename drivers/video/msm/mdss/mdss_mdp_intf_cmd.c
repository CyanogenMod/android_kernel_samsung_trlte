...
#define STOP_TIMEOUT_FOR_ALPM 1000
		...
		int panel_power_state)
{
	...
	/*
	 * Use different timeout value because alpm and non-alpm generate
	 * different period of TE signal
	 */
	if (alpm_data->alpm_status && alpm_data->alpm_status(CHECK_CURRENT_STATUS))
		stop_timeout = STOP_TIMEOUT_FOR_ALPM;
	else
